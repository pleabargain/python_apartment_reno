...
def generate_cost_report(manager: RenovationManager, export: bool = False, user_name: str = None) -> str:
    ...
    try:
        ...
        if export:
            ...
            filename = f"room_cost_report_{current_date}.md"
            
            # Add user name to report if provided
            if user_name:
                md_content.insert(1, f"Generated by: {user_name}\n\n")
                md_content.insert(2, f"Date: {current_date}\n\n")
                final_content = ''.join(md_content)
            
            with open(filename, 'w') as f:
                f.write(final_content)
            print(f"\nCost report exported to {filename}")
...
def view_common_data(manager: RenovationManager, choice: str):
    ...
    if choice == 'r':
        print(generate_cost_report(manager))
        export = input("\nWould you like to export this report as markdown? (y/n): ").lower() == 'y'
        if export:
            user_name = input("\nPlease enter your name: ").strip()
            generate_cost_report(manager, export=True, user_name=user_name)
    else:
        try:
            value = manager.get_nested_value(path)
            print(manager.format_value(value))
