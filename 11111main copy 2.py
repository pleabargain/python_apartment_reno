...
def show_room_menu(manager: RenovationManager):
...
def show_room_details(manager: RenovationManager, room_name: str):
    ...
    if 'budget' in room_data:
        ...
        print(f"Budget: {budget.get('amount', 0):,.2f} AED")
    ...
    for key, value in room_data.items():
        if isinstance(value, dict) and key != 'budget':
            sections.append(key)
            if 'cost' in value:
                print(f"{len(sections)}. {key.replace('_', ' ').title()} (Cost: {value['cost']:,.2f} AED)")
            else:
                print(f"{len(sections)}. {key.replace('_', ' ').title()}")
...
def handle_add_update(manager: RenovationManager):
    ...
    print("\nEnter room number (or 'show all' for all paths):")
    try:
    ...
    if user_input.lower() == 'show all':
        show_available_paths(manager, show_all=True)
        print("\nEnter path to add/update:")
        try:
            path = input().strip()
        except EOFError:
            return
        ...
        try:
            ...
            if 0 <= room_index < len(room_names):
                ...
                try:
                    ...
                    if 0 <= section_index < len(sections):
                        ...
                        path_parts = ['rooms', room_name, section]
                    else:
            ...
            path_parts = user_input.split('/')
    
    if not path_parts:
        print("Error: Invalid path")
        return

    # Get path info and handle value update
    ...
    try:
        ...
        print(f"Successfully updated {'/'.join(path_parts)}")
    ...
    while True:
        ...
        try:
            ...
            if choice == '1':
                ...
                try:
                    value = manager.get_nested_value(path_parts)
                    print("\nData at path:", '/'.join(path_parts) if path_parts else 'root')
                    print("-" * 60)
                    print(manager.format_value(value))
                except KeyError as e:
                    print(f"Error: {e}")
                ...
                try:
                    ...
                    print(f"Successfully deleted {'/'.join(path_parts)}")
                except KeyError as e:
                    print(f"Error: {e}")
