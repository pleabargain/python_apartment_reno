...
def show_available_paths(manager: RenovationManager, current_path: str = "", show_all: bool = False):
    ...
    if show_all:
        ...
        print("\nAvailable dictionary paths (numbered for easy access):")
        dict_paths = []
        for path in paths:
            info = manager.get_path_info(path.split('/'))
            if info and info['is_dict']:
                dict_paths.append(path)
        
        for i, path in enumerate(dict_paths, 1):
            print(f"  {i}. {path}")
        
        print("\nTip: Enter a number or full path. Use 'show all' to see all available paths.")
        return dict_paths
...
def handle_add_update(manager: RenovationManager):
    ...
    dict_paths = show_available_paths(manager)
    
    print("\nEnter path to add/update (number, path, or 'show all'):")
    try:
    ...
    if user_input.lower() == 'show all':
        ...
        try:
            path_index = int(user_input) - 1
            if 0 <= path_index < len(dict_paths):
                path = dict_paths[path_index]
            else:
                path = user_input
        except ValueError:
            path = user_input
    ...
    while True:
        ...
        try:
            ...
            if choice == '1':
                ...
                print("\nEnter path to view (number, path, or 'show all'):")
                try:
                    user_input = input().strip()
                except EOFError:
                    break
                ...
                print("\nEnter path to delete (number, path, or 'show all'):")
                try:
                    user_input = input().strip()
                except EOFError:
                    break
